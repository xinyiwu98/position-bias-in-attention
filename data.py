...
class SeqDataset(Dataset):
    ...
    def generate_content(self, classes, class_mean, class_label):
        ...
        if self.data_type == "fixed":
            ...
            eta = torch.normal(mean=torch.tensor(0), std=torch.tensor(1/self.D), size=(self.D,)) # eta
        ...
        for key in classes[:-1]:
            content_list.append(((class_mean[key] + eps * eta) / normalization_factor).unsqueeze(0))
            # content_list.append(class_mean[key].unsqueeze(0))
            content_list.append(self.label_content[class_label[key]].unsqueeze(0))
          
        # Handle the query class
        query_class = classes[-1]
        query_content = ((class_mean[query_class] + eps * eta) / normalization_factor).unsqueeze(0)
        # query_content = class_mean[query_class].unsqueeze(0)
